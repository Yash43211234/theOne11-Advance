// src/components/CompetitionsSection.js
import React from 'react';
import styles from './CompetitionsSection.module.css';

const CompetitionCard = ({ imgSrc, imgAlt, title, description, linkHref }) => (
    <div className={styles.card}>
        <img src={imgSrc} alt={imgAlt} className={styles.cardImage} />
        <div className={styles.cardContent}>
            <h3 className={styles.cardTitle}>{title}</h3>
            <p className={styles.cardDescription}>{description}</p>
            <a href={linkHref} className={`${styles.cardButton} btn-primary`}>Register Now</a>
        </div>
    </div>
);
...
const CompetitionsSection = () => {
    ...
    return (
        <section id="competitions" className={styles.competitionsSection}>
            <img src="https://placehold.co/600x600/171616/f81a26/png?text=Vinyl+Disc" alt="Animated Vinyl Disc" className={styles.competitionBackgroundVinyl} />
